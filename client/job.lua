...
local function SetWeaponSeries()
    for k, _ in pairs(Config.Items.items) do
        if k < 6 then
            Config.Items.items[k].info.serie = tostring(QBCore.Shared.RandomInt(2) ..
...
function MenuGarage(currentSelection)
    ...
    lib.registerContext({
        id = 'open_police_menu_garage',
        title = Lang:t('menu.garage_title'),
        options = vehicleMenu
    })
    lib.showContext('open_police_menu_garage')
end
...
RegisterNetEvent('police:client:EvidenceStashDrawer', function(data)
    ...
    if #(pos - takeLoc) <= 1.0 then
        local drawer = lib.inputDialog(Lang:t('info.evidence_stash', { value = currentEvidence }),
            {
                { type = 'number', label = 'slot', placeholder = Lang:t('info.slot') }
            }
        )
        if not drawer then return end
        TriggerServerEvent("inventory:server:OpenInventory", "stash",
            Lang:t('info.current_evidence', { value = currentEvidence, value2 = drawer[1] }),
            { maxweight = 4000000, slots = 500 }
        )
        ...
        if isPointInside then
            if PlayerJob.type == "leo" and onDuty then
                ...
                lib.registerContext({
                    ...
                    id = 'open_police_evidence_drawer',
                    options = {
                        {
                            title = 'Police Evidence Drawer',
                ...
                lib.showContext('open_police_evidence_drawer')
            end
        else
            lib.hideContext()
        end
            ...
            if onDuty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    ...
                    lib.registerContext({
                        ...
                        id = 'open_police_impound_menu',
                        options = {
                            {
                                title = 'Police Impounded Vehicles',
                    ...
                    lib.showContext('open_police_impound_menu')
                end
            end
        else
            inImpound = false
        ...
        if isPointInside then
            ...
            if onDuty and PlayerJob.type == 'leo' then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    ...
                    lib.showContext('open_police_vehicle_menu')
                end
            end
        else
            inGarage = false
